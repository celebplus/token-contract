...
contract('CelebPlusToken', ([creator, receiver, thirdParty]) => {
  ...
  const transferAmount = new BigNumber('100e18');  // 100 CELEBPLUS tokens
  const approveAmount = new BigNumber('10e18');  // 10 CELEBPLUS tokens


  beforeEach(async () => {
    this.token = await CelebPlusToken.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY, { from: creator });
  });
  ...
  describe('ERC20::Standard Token', () => {
    ...
    it('assigns the initial total supply to the creator', async () => {
      ...
      const logs = decodeLogs(receipt.logs, CelebPlusToken, this.token.address);
