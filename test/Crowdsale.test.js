...
const CelebPlusToken = artifacts.require('CelebPlusToken');
const CelebPlusCrowdsale = artifacts.require('CelebPlusCrowdsale');
...
contract('CelebPlusCrowdsale', ([creator, wallet, funder, thirdParty, thirdPartyAlt, address5, address6, address7, address8]) => {
  ...
  const VESTING_TOKEN_AMOUNT = new BigNumber('4e22'); // 40,000 CELEBPLUS
  const BONUS_RATE = new BigNumber(2500); // 25% Bonus rate which is multiplied by 100 for better resolution

  // These values are for testing purposes. The values set in CelebPlusCrowdsale 
  ...
  const vestAmount_funder = new BigNumber('2.5e21'); // 2,500 CELEBPLUS
  const vestAmount_thirdParty = new BigNumber('1e21'); // 1,000 CELEBPLUS
  const vestAmount_thirdPartyAlt = new BigNumber('5e21'); // 5,000 CELEBPLUS


  beforeEach(async () => {
    this.token = await CelebPlusToken.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY);
    this.crowdsale = await CelebPlusCrowdsale.new(EXCHANGE_RATE, wallet, this.token.address, TOTAL_SALE_CAP);
  ...
  describe('Crowdsale::Lockable', () => {
    ...
    const lockedAmount = new BigNumber('200e18'); // 200 CELEBPLUS tokens
